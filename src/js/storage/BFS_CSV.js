    ...
    if(csv.xIncrement) {
        if(!csv.lastX) {
            ...
            if(newData[csv.header[2]]) {
                if(Array.isArray(newData[csv.header[2]])) x = csv.lastX + csv.xIncrement*(newData[csv.header[2]]).length;
                else x = csv.lastX + csv.xIncrement;
            }
            else if(newData[csv.header[0]]) {
                if(Array.isArray(newData[csv.header[0]])) x = csv.lastX + csv.xIncrement*(newData[csv.header[0]]).length;
                else x = csv.lastX + csv.xIncrement;
            }
            else x = csv.lastX + csv.xIncrement;
...
export const createCSV = (
    filename,
    header,
    toFixed=5, //limit decimals (saves a ton of memory), set to 0 for integers
    bufferSize=0, //accumulate a certain number of lines before writing to BFS? (necessary for > 100 updates/sec fyi)
    xIncrement //fixed time increment for newlines?
) => {
